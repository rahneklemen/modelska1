...
def f(x, a,b,c,d):
    return a*np.exp(-x*b)+c*np.exp(-d*np.sqrt(x))
...
popt, pcov = curve_fit(f, x, y,p0=(8000,0.001,5000,0.0001),sigma=y_napaka,maxfev=10000)
a=popt[0]
b=popt[1]
c=popt[2]
d=popt[3]
...
plt.xlim(0,2160)
plt.ylim(0,14000)
plt.legend(['meritve',r'prilagajanje $N=A e^{- \lambda_1 t} + B e^{- \lambda_2 \sqrt{t}}$'])
plt.xlabel('čas')
plt.ylabel('N (število sunkov)')
plt.savefig('druga_2.png')
plt.show()
